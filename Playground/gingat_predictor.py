...
# from dgllife.model.gnn import GATv2, GAT
# from dgllife.model.readout import WeightedSumAndMax
# from gatv2_custom import GATv2_custom
from supergatv2 import SuperGATv2
from weighted_sum_and_max_on_gpu import WeightedSumAndMaxonGPU
from gin_attn import GIN_attn
from gatv2_custom import GATv2_custom


# pylint: disable=W0221
class GINGATPredictor(nn.Module):
    ...
    def __init__(
        ...
        super(GINGATPredictor, self).__init__()

        if gin_attn_types is None:
            self.gin_attn_types = ["NONE", "NONE"]
        self.gin_part = GIN_attn(
            in_feats=74,
            gin_attn_types=gin_attn_types,
            out_feats=74
        )

        # self.gnn = GATv2_custom(in_feats=in_feats,
        ...
        #                         agg_modes=agg_modes
        #                         )
        self.gnn = SuperGATv2(in_feats=74,
                              hidden_feats=hidden_feats,
                              num_heads=num_heads,
                              feat_drops=feat_drops,
                              attn_drops=attn_drops,
        ...
        self.readout = WeightedSumAndMaxonGPU(gnn_out_feats)
        # self.predict = MLPPredictor(2 * gnn_out_feats, predictor_out_feats, n_tasks, predictor_dropout)
        self.predict = MLPPredictor(gnn_out_feats, predictor_out_feats, 12, predictor_dropout)
        pass
    ...
    def forward(self, bg, feats, get_attention=False):
        ...
        if get_attention:
            node_feats = self.gin_part(bg, feats)
            node_feats, attentions = self.gnn(bg, node_feats, get_attention=get_attention)
